...
class RepConv(nn.Module):
    ...
    原文链接：https://blog.csdn.net/pengxiang1998/article/details/128307956
    '''
    ...
    def forward(self, inputs):
        ...
        # print("self.act(self.rbr_dense(inputs) + self.rbr_1x1(inputs) + id_out).shape",self.act(self.rbr_dense(inputs) + self.rbr_1x1(inputs) + id_out).shape)
...
class NMS(nn.Module):
    # Non-Maximum Suppression (NMS) model
...
class ChannelAttentionModule(nn.Module):
    def __init__(self, c1, reduction=16):
        ...
        self.shared_MLP = nn.Sequential(
            nn.Linear(in_features=c1, out_features=mid_channel, bias=False),
            nn.LeakyReLU(0.1, inplace=True),
            nn.Linear(in_features=mid_channel, out_features=c1, bias=False)
        )
        self.act = nn.Sigmoid()
        # self.act=nn.SiLU()

    '''
    假设x.shape=[1,1024,32,32]
    self.avg_pool(x).shape = [1,1024,1,1]
    self.avg_pool(x).view(x.size(0), -1).shape = [1,1024]
    self.shared_MLP(self.avg_pool(x).view(x.size(0), -1)).shape = [1,1024]     //64 = 1024//16
    self.shared_MLP(self.avg_pool(x).view(x.size(0), -1)).unsqueeze(2).unsqueeze(3).shape = [1,1024,1,1]

    out.shape = [1,1024,1,1]
    '''
...
class SpatialAttentionModule(nn.Module):
    ...
    '''
    假设x.shape=[1,1024,32,32]
    avgout.shape=[1,1,32,32]
    maxout.shape=[1,1,32,32]
    torch.cat([avgout, maxout], dim=1).shape = [1,2,32,32]
    out.shape=[1,1,32,32]
    '''
...
class PixelAttention(nn.Module):
    ...
    '''
    x.shape = [1,1024,32,32]
    x.unsqueeze(dim=2).shape = [1,1024,1,32,32]
    patten1.shape = [1,1024,32,32]
    pattn1.unsqueeze(dim=2).shape = [1,1024,1,32,32]
    x2 = torch.cat([x, pattn1], dim=2).shape = [1,1024,2,32,32]
    x2 = Rearrange('b c t h w -> b (c t) h w')(x2).shape = [1,2048,32,32]
    patten2.shape = [1,1024,32,32]
...
class CBAM(nn.Module):
    def __init__(self, c1, c2=None):
        ...
        # self.pixel_attention = PixelAttention(c1)

    '''
    input.shape = [1,1024,32,32]
    ca.out = [1,1024,1,1]
    ca.out = [1,1,32,32]
    patten1 = [1,1024,32,32]
    '''
    ...
    def forward(self, x):
        ...
        # patten1 = ca + sa
        # patten2 = self.pixel_attention(x, patten1)
        # res = x + patten2
        return sa
...
class DEABlock(nn.Module):
    def __init__(self, conv, dim, kernel_size, reduction=8):
        ...
        self.ca = ChannelAttentionModule(dim, reduction)
...
# 初始想法 加在RepConv后
class GCN(nn.Module):
    def __init__(self, c1, c2, num_classes=5, t=0.4,
                 adj_file='/home8T/swx/yolov7/datasets/VOC_four_noVal/word/voc_adj.pkl',
                 inp_name='/home8T/swx/yolov7/ML_GCN/word/glove_wordEmbedding.pkl'):  # model
        # t=0.02
    ...
    def forward(self, input_feature):
        ...
        return input_feature * attention_weight
...
class feature_fusion(nn.Module):
    def __init__(self, c2):
        ...
        self.m = 0.80
        self.w = torch.nn.Parameter(torch.FloatTensor([self.m]), requires_grad=True)
        self.w = torch.nn.Parameter(self.w, requires_grad=True)

        self.conv = nn.Conv2d(512, 512, 3, stride=1, padding=1)
        self.up1 = nn.ConvTranspose2d(1024, 256, 3, stride=2, padding=1, output_padding=1)
        self.up2 = nn.ConvTranspose2d(1024, c2, 3, stride=2, padding=1, output_padding=1)
        self.act = nn.ReLU()

    #### x1 是目标层      x2和x3是高级语义信息

    def forward(self, feature_list):
        x1 = self.conv(feature_list[0])

        x2_up = self.up1(feature_list[1])
        x2_up = F.normalize(x2_up, p=2, dim=1)
        x2_up = self.act(x2_up)

        x3 = self.up2(feature_list[2])
        x3 = self.w * x3 + (1 - self.w) * feature_list[1]
        x3 = self.up1(x3)
        x3 = F.normalize(x3, p=2, dim=1)
        x3 = self.act(x3)

        x = torch.cat((x1, x2_up, x3), dim=1)
        return x


################## end of feature fusion ###########################
