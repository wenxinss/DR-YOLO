...
class Model(nn.Module):
    ...
    def forward_once(self, x, profile=False):
        if isinstance(x, list):
            IsTrain = x[0].requires_grad
        else:
            IsTrain = x.requires_grad
        y, dt = [], []  # outputs
        for m in self.model:
    ...
    def nms(self, mode=True):  # add or remove NMS model
    ...
    def autoshape(self):  # add autoShape model
...
'''
该配置文件包含 nc , model depth multiple, layer channel multiple , anchors , backbone , head
'''


def parse_model(d, ch):  # model_dict, input_channels(3)
    logger.info('\n%3s%18s%3s%10s  %-40s%-30s' % ('', 'from', 'n', 'params', 'model', 'arguments'))
    # 根据字典的键值对进行取值
    anchors, nc, gd, gw = d['anchors'], d['nc'], d['depth_multiple'], d[
        'width_multiple']  # 深度可以理解为网络的层数; 宽度可以理解为网络层输出通道的大小
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, model, args
        ...
        if m in [nn.Conv2d, Conv, RobustConv, RobustConv2, DWConv, GhostConv, RepConv, RepConv_OREPA, DownC,
            ...
            c1, c2 = ch[f], args[0]
            if c2 != no:
                c2 = make_divisible(c2 * gw, 8)  # 向上取整*8
            args = [c1, c2]
        elif m in [Mix]:
            c2 = args[1]
        else:
        ...
        m_ = nn.Sequential(*[m(*args) for _ in range(n)]) if n > 1 else m(*args)  # model
        # --------------------------输出到terminal的模型信息，进行记录输出----------------------------------
        t = str(m)[8:-2].replace('__main__.', '')  # model type
        np = sum([x.numel() for x in m_.parameters()])  # number params
        m_.i, m_.f, m_.type, m_.np = i, f, t, np  # attach index, 'from' index, type, number params
        logger.info('%3s%18s%3s%10.0f  %-40s%-30s' % (i, f, n, np, t, args))  # print
        save.extend(x % i for x in ([f] if isinstance(f, int) else f) if x != -1)  # append to savelist 输出
        # ----------------------------------------！！！------------------------------------------------
        # layers为整个model的层，将模块的层添加到整个layers层
        layers.append(m_)
        # ch在此定义，即每一层输出通道数的列表，一共24个元素，对应yolov7层每层通道数
        if i == 0:
            ch = []
        ch.append(c2)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='/home8T/swx/yolov7/cfg/training/V4.yaml',
                        help='model.yaml')  # _m_fasterNet2
    ...
    # img = torch.rand(4, 3, 640, 640).to(device)
