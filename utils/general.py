...
def check_git_status():
    # Recommend 'git pull' if data is out of date
...
#     iou_mean = 1.
#     monotonous = False
#     _momentum = 1 - pow(0.05, 1 / (4595 * 16))   # 1 - 0.5 ** (1 / 7000)
#     _is_train = True
#
...
#
#     @classmethod
#     def _scaled_loss(cls, self, gamma=1.2, delta=6):    # 3,1.9
...
def non_max_suppression(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if nc == 1:
            ...
            x[:, 5:] *= x[:, 4:5]  # conf = obj_conf * cls_conf 条件概率，每个类别的概率还需要乘以是否是目标这类的概率。
