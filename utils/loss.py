...
class ComputeLoss:
    ...
    def __init__(self, model, autobalance=False):
        ...
        det = model.module.model[-1] if is_parallel(model) else model.model[-1]  # Detect() model
    ...
    def __call__(self, p, targets):  # predictions, targets, model
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                #     if len(iou) == 2:
                #         lbox += (iou[i].detach() * (1 - iou[0])).mean()
                #         iou = iou[0]
                #     else:
...
class ComputeLossOTA:
    ...
    def build_targets(self, p, targets, imgs):
        # p为模型输出的预测值，有3层，size=[(bs,3,20,20,85),(bs,3,40,40,85),(bs,3,80,80,85)], targets的size=(nt, 6)=(GT数量，6)=(nt, index c x y w h)
        #indices, anch = self.find_positive(p, targets)
        indices, anch = self.find_3_positive(p, targets)#compute_loss()的build_targets，找到正样本, indices[i]=(image, anchors index, grid indices)
        ...
        for batch_idx in range(p[0].shape[0]):#对每一张图片进行匹配
        
            b_idx = targets[:, 0]==batch_idx# 找到batch_idx的目标（找到每张图像对应的GT）
            this_target = targets[b_idx]#取出该目标 this_target.shape=(标注的GT数量，6)=(nt, index c x y w h)  当前图像里的标注框GT
            if this_target.shape[0] == 0:#目标数量是否为0
                continue
                
            txywh = this_target[:, 2:6] * imgs[batch_idx].shape[1]# this_target*wh，还原GT框, shape=(nt, 4)    得到实际大小
            txyxy = xywh2xyxy(txywh)#转换成(top left xy, bottem right xy), shape=(nt,4)    变成左上角右下角
            ...
            for i, pi in enumerate(p):#p size=[(bs,3,20,20,85),(bs,3,40,40,85),(bs,3,80,80,85)]
                
                b, a, gj, gi = indices[i]# indices[i]=(image, anchors index, grid indices), 当前层匹配的正样本，image index, anchors index, gj, gi
                idx = (b == batch_idx)# 得到当前图片id的所有正样本id
                b, a, gj, gi = b[idx], a[idx], gj[idx], gi[idx] #得到当前图片id的所有正样本
                all_b.append(b)#image index
                all_a.append(a)#anchors index
                all_gj.append(gj) #gj
                all_gi.append(gi) #gi
                all_anch.append(anch[i][idx])#anchors
                from_which_layer.append((torch.ones(size=(len(b),)) * i).to(device))# torch.ones(size=(len(b),)) * i=[i,i,i,i,i]
                
                fg_pred = pi[b, a, gj, gi]   # 得到预测值, shape=(len(b), 85)
                p_obj.append(fg_pred[:, 4:5])# obj预测值
                p_cls.append(fg_pred[:, 5:])# cls预测值
                
                grid = torch.stack([gi, gj], dim=1)# [gi, gj], shape=(len(b), 85)
                pxy = (fg_pred[:, :2].sigmoid() * 2. - 0.5 + grid) * self.stride[i] #/ 8. 预测的xy值
                #pxy = (fg_pred[:, :2].sigmoid() * 3. - 1. + grid) * self.stride[i]
                pwh = (fg_pred[:, 2:4].sigmoid() * 2) ** 2 * anch[i][idx] * self.stride[i] #/ 8. 预测的wh值
                pxywh = torch.cat([pxy, pwh], dim=-1)# [x,y,w,h], shape=(len(b), 4)
                pxyxy = xywh2xyxy(pxywh)# [x,y,x,y], shape=(len(b), 4)
                pxyxys.append(pxyxy)
            
            pxyxys = torch.cat(pxyxys, dim=0)# 将本张图片的所有正样本合并, shape=(P, 4)
            if pxyxys.shape[0] == 0:
                continue
    ...
    def find_3_positive(self, p, targets):
        # Build targets for compute_loss(), input targets(image,class,x,y,w,h)
        na, nt = self.na, targets.shape[0]  # number of anchors, targets     anchor的数量和targets的数量
        indices, anch = [], []
        gain = torch.ones(7, device=targets.device).long()  # normalized to gridspace gain  初始化为1：主要目的就是将坐标位置还原到特征尺度
        ai = torch.arange(na, device=targets.device).float().view(na, 1).repeat(1, nt)  # same as .repeat_interleave(nt)
        targets = torch.cat((targets.repeat(na, 1, 1), ai[:, :, None]), 2)  # append anchor indices
        ...
        for i in range(self.nl):
            ...
            # Match targets to anchors  看GT放在哪个输出特征层上合适
            t = targets * gain  # 把归一化标签映射到特征图上
            if nt:
                # Matches
                r = t[:, :, 4:6] / anchors[:, None]  # wh ratio     目的是找到和GT有相似宽高比的anchor
                j = torch.max(r, 1. / r).max(2)[0] < self.hyp['anchor_t']  # 0.25<比例<4 才会被保留 compare
                # j = wh_iou(anchors, t[:, 4:6]) > model.hyp['iou_t']  # iou(3,n)=wh_iou(anchors(3,2), gwh(n,2))
                t = t[j]  # filter   初步筛选

                # Offsets
                gxy = t[:, 2:4]  # grid xy  到左上角的距离
                gxi = gain[[2, 3]] - gxy  # inverse 到右下角的距离
                j, k = ((gxy % 1. < g) & (gxy > 1.)).T  # 离左上角近的选出来 而且不能是边界
                l, m = ((gxi % 1. < g) & (gxi > 1.)).T  # 离右下角近的选出来 而且不能是边界
                j = torch.stack((torch.ones_like(j), j, k, l, m))   # 5个 因为自己所在位置一定为true    1+2
                t = t.repeat((5, 1, 1))[j]  # 相当于原来就一个 现在还要考虑两个邻居 targets必然增多
                offsets = (torch.zeros_like(gxy)[None] + off[:, None])[j]
            else:
                t = targets[0]
                offsets = 0
            ...
            # Define
            b, c = t[:, :2].long().T  # image(batch的索引), class
            gxy = t[:, 2:4]  # grid xy
            gwh = t[:, 4:6]  # grid wh
            gij = (gxy - offsets).long()    # 偏移后 整数部分就是格子的索引
            gi, gj = gij.T  # grid xy indices

            # Append
            a = t[:, 6].long()  # 每一个target 对应的anchor indices
            indices.append((b, a, gj.clamp_(0, gain[3] - 1), gi.clamp_(0, gain[2] - 1)))  # image（bitch的索引）, anchor（对应的anchor的索引）, grid indices（格子所在的位置）
            anch.append(anchors[a])  # anchors
