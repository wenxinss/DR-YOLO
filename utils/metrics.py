...
def compute_ap(recall, precision, v5_metric=False):
    ...
    if v5_metric:  # New YOLOv5 hazy_metric, same as MMDetection and Detectron2 repositories
        mrec = np.concatenate(([0.], recall, [1.0]))
    else:  # Old YOLOv5 hazy_metric, i.e. default YOLOv7 hazy_metric
...
def plot_mc_curve(px, py, save_dir='mc_curve.png', names=(), xlabel='Confidence', ylabel='Metric'):
    ...
    if 0 < len(names) < 21:  # display per-class legend if < 21 classes
        for i, y in enumerate(py):
            ax.plot(px, y, linewidth=1, label=f'{names[i]}')  # plot(confidence, hazy_metric)
    else:
        ax.plot(px, py.T, linewidth=1, color='grey')  # plot(confidence, hazy_metric)
